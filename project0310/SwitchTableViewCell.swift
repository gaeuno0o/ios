...
//  project0310
//
//  Created by 김가은 on 2021/03/26.
//

import Foundation
import UIKit

class SwitchTableViewCell: UIViewController, UITableViewDelegate, UITableViewDataSource {
    
    @IBOutlet weak var tableView: UITableView!
    
    static let identifier = "SwitchTableViewCell"
    let cellIdentifier: String = "cell"
    private let lists: [String] = ["내 정보", "차단 목록", "보안", "공지사항", "문의", "정보", "아웃페이 탈퇴"]
    private let noti: [String] = ["결제 요청", "결제만료 임박", "기념일"]
    
    private let sections: [String] = [" ", "푸쉬 알림"]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell { //row에 해당되는 cell을 알려죵
        ...
        let text: String = indexPath.section == 0 ? lists[indexPath.row] : noti[indexPath.row]
